...
namespace XamlFlair.Extensions
{
	internal static class AnimationSettingsExtensions
	{
		...
		internal static AnimationSettings ApplyOverrides(this AnimationSettings settings, AnimationSettings other)
		{
			...
			updated.OffsetX = offsetX != 0 ? offsetX : settings.OffsetX;

			var offsetY = other.OffsetY;
			updated.OffsetY = offsetY != 0 ? offsetY : settings.OffsetY;
			...
			var origin = other.RenderTransformOrigin;
			updated.RenderTransformOrigin = origin != AnimationSettings.DEFAULT_TRANSFORM_ORIGIN ? origin : settings.RenderTransformOrigin;
